...
public class MemberListServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try {
			 ...
			 * 1) redirect : 브라우저한ㅌ테 자른 주소로 새로 접속해 
			 * 				request를 공유할 수 있다. 
			 * 				(기존 request를 가지고 요청하는 것이 아닌, 새로운 요청이기 때문에)
			 * 				<브라우저 url창은 기존 주소로 접속했다가, 새로운 주소로 표시됨!>
			 * 
			 * 2) 기존 request을 전달하는 요청
			 * 				: 기존 request가 아직 브라우저에 응답으로 가지 않고,
			 * 				서버 내부에서 경로 이동만 하는 방식* 브라우저가청므 접속한 주소가 브라우저에 그대로 표현된다. 
			 * 				(다만 서버 내부에서는 경로를 이동)
			 *	a)include : 일시 호출하고 다시 호출한 곳으로 제어권 넘어온다.
